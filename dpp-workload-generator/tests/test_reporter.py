...
def test_reporter_basic_flow(tmp_path):
    reporter = ScenarioReporter("s1", "Offline Interpretability", output_dir=tmp_path)
    ...
    assert "# Scenario S1: Offline Interpretability" in content
...
def test_reporter_exception_handling(tmp_path):
    reporter = ScenarioReporter("s1", "Exception test", output_dir=tmp_path)
