...
def test_reporter_basic_flow(tmp_path):
    reporter = ScenarioReporter("s5", "Offline Validation After Platform Unavailability", output_dir=tmp_path)
    ...
    assert "# Scenario S5: Offline Validation After Platform Unavailability" in content
...
def test_reporter_exception_handling(tmp_path):
    reporter = ScenarioReporter("s5", "Exception test", output_dir=tmp_path)
