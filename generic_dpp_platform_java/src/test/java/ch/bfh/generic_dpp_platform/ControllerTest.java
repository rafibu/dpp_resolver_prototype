...
public abstract class ControllerTest {
    ...
    protected String sendRequestAndExpectString(MockHttpServletRequestBuilder request) throws Exception {
        return sendRequestAndExpectString(request, HttpStatus.OK);
    }

    protected <T> T sendRequestAndExpectObject(MockHttpServletRequestBuilder request, Class<T> clazz) throws Exception {
        return sendRequestAndExpectObject(request, clazz, false, HttpStatus.OK);
    }

    protected <T> T sendRequestAndExpectObject(MockHttpServletRequestBuilder request, Class<T> clazz, HttpStatus status) throws Exception {
        return sendRequestAndExpectObject(request, clazz, false, status);
    }

    protected String sendRequestAndExpectString(AbstractMockHttpServletRequestBuilder request, HttpStatus status) throws Exception {
        return sendRequest(request)
                .andExpect(status().is(status.value()))
                .andReturn().getResponse().getContentAsString();
    }

    protected <T> T sendRequestAndExpectObject(AbstractMockHttpServletRequestBuilder request, Class<T> clazz, boolean ignoreObjectMappingError, HttpStatus status) throws Exception {
        return createGson(ignoreObjectMappingError).fromJson(
                sendRequestAndExpectString(request, status), clazz);
    }

    protected <T> T sendRequestAndExpectObject(MockHttpServletRequestBuilder request, Class<T> clazz, boolean ignoreObjectMappingError) throws Exception {
     ...
     */
    protected static Gson createGson(boolean ignoreError) {
